function __bobthefish_colors -S -a color_scheme -d 'Define colors used by bobthefish'
  switch "$color_scheme"
    ...
    case 'terminal' 'terminal-dark*'
      ...
      set -x color_initial_segment_exit     black brred --bold
      set -x color_initial_segment_private  black black
      set -x color_initial_segment_su       black green --bold
      set -x color_initial_segment_jobs     black blue --bold

      set -x color_path                     green black --bold
      set -x color_path_basename            green black --bold
      set -x color_path_nowrite             magenta $colorfg
      set -x color_path_nowrite_basename    magenta $colorfg --bold

      set -x color_repo                     black white
      set -x color_repo_work_tree           black green --bold
      set -x color_repo_dirty               black brred
      set -x color_repo_staged              black yellow
